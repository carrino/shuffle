...
const good = {
  ...
  deck: 'sleeved-100',
  intendedSplit: 30,
  string: 'UT'.repeat(15) + 'T'.repeat(70),
  n: 100,
};
...
describe('schema.validateRecord', () => {
  ...
  it('rejects string length !== n', () => {
    const r = validateRecord({ ...good, string: 'UT' });
    expect(r.ok).toBe(false);
    if (!r.ok) expect(r.errors.join()).toContain('length');
  });

  it('rejects characters outside {U,T}', () => {
    const r = validateRecord({ ...good, string: 'RX'.repeat(50) });
    expect(r.ok).toBe(false);
    if (!r.ok) expect(r.errors.join()).toContain('other than U and T');
  });
  ...
  it('rejects missing fields, bad timestamps, bad n', () => {
    ...
    expect(validateRecord('UUTT').ok).toBe(false);
  });
...
describe('store.parseJsonl', () => {
  it('parses valid lines, skips and reports invalid ones', () => {
    const text = [
      JSON.stringify(good),
      'not json at all {',
      JSON.stringify({ ...good, string: 'UUT' }),
      '',
